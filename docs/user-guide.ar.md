...
كلّ التقاط يكتب ملفَّي PDF حسّاسَين للّغة في مجلّد فرعي اسمه `reports/`:
...
- شجرة `downloads/` تُرايي تخطيط مجلّد العنصر على القرص بدقّة: كلّ مجلّد `{stem}/` يحوي الوسائط (إن وُجدت)، و `meta.json` + `.sig`، و `checksums.txt`، وثلاثيّ لقطة الصفحة (`mhtml`/`png`/`warc.gz`)، وزوج `reports/{stem}.manifest.pdf` + `reports/{stem}.report.pdf`.
