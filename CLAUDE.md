...
/downloads/{case_slug}/{stem}/Captures/{stem}.page.frozen.html  ← single-file frozen DOM (v0.12)
...
| `Captures/{stem}.page.frozen.html` | Best-effort      | Frozen single-file HTML view (v0.12). Post-JS-rendered DOM with inlined computed styles, scripts/stylesheets stripped, images embedded as data: URIs (three-tier size ladder; falls back to absolute URLs above 25 MB hard cap). A derived view, not a substitute for MHTML/WARC — those remain the forensic source of record. Hashed and bound transitively by `meta.json.sig`. Absent only when the size budget is busted; the `meta.json.capture.frozen_html.error` field carries the reason.                                                                                                                                                                                                                                                                                              |
...
`{stem}.meta.json` is the canonical record. Schema lives at `/app/schemas/meta.schema.json` and is versioned (`"schema_version": 11` for new captures; v2–v10 records continue to validate). When the schema changes, write a migration. v2 (hardening pass) adds:
...
### Frozen single-file HTML view (v0.12)

archive.today (archive.ph / archive.is) is closed-source and anonymous-operator but its output technique is well-understood: render the URL in a real browser, serialize the post-JS DOM, inline computed styles, embed images as `data:` URIs, strip JavaScript. The result is a single `.html` file that any browser — *any year* — will render even if the source site is gone, the source JavaScript breaks, or MHTML support is dropped. Capsule already does *more* than archive.today on the forensic side (Ed25519 signatures, audit chain, hash-bound `meta.json`) but until v0.12 the only "viewable" artifact was the MHTML, and MHTML support is decaying: Firefox dropped it, Chrome's is behind a flag. A recipient opening an evidence bundle in 2030 might find the `.mhtml` unrenderable. v0.12 closes this gap.

- **New artifact role `page_frozen_html`** ([`Captures/{stem}.page.frozen.html`](app/postprocess.py)) — a single self-contained HTML file alongside MHTML/PNG/WARC. Explicitly a **derived view, not a substitute** for MHTML or WARC; those remain the forensic source of record. The frozen HTML is the format a recipient opens first when they just want to *see* the page. Hashed and bound transitively by `meta.json.sig`.
- **New module [`app/frozen_html.py`](app/frozen_html.py)** — async `generate(page, work_dir)` runs an in-browser `page.evaluate` DOM walker that emits the live tree as a serialized HTML string with computed styles inlined via `style=""` attributes. Strips every `<script>`, `<link rel=\"stylesheet\">`, `<style>`, plus `@font-face` declarations (count recorded). Preserves `<noscript>` content inline (often the most archive-faithful representation — same behavior as archive.today). Preserves `<bdi>` elements and `dir` attributes per §4.5 bidi.
- **Forensic stance — three insertion-point decisions:**
  1. **Generated AFTER the animation-freeze stylesheet is removed.** The PNG screenshot uses an animation-freeze CSS layer with `animation-play-state: paused !important`. If the frozen HTML walked the DOM with that CSS active, every element's inlined `style=""` would silently bake in the paused state — a capture-side mutation invisible to recipients. Generating after removal means the inlined computed styles reflect the page's source CSS, not Capsule's overlay. A visual PNG/frozen-html disagreement on a running carousel is honest divergence (CLAUDE.md §13 #14).
  2. **Generated AFTER the WARC writer closes.** The image-body fetches the frozen-html generator issues via `page.request.fetch()` are derivation work, not page-load events. Closing the WARC writer first means those requests don't pollute the WARC's record of what the page itself loaded.
  3. **Image bodies fetched via [`page.request.fetch()`](app/frozen_html.py)** (Playwright's APIRequestContext at the browser layer) — sidesteps the CORS failures an in-page `fetch()` would hit on cross-origin CDN images. The bytes are deduped per-URL so reused logos / spacers fetch once.
- **Three-tier size ladder** with a 10 MB soft target and a 25 MB hard cap:
  - **`full`** — inline images up to 256 KB each (typical page)
  - **`small_only`** — retry with 64 KB per-image cap (image-heavy)
  - **`external`** — leave every `<img src>` as an absolute URL (pathological)
  - **hard cap** — even the `external` tier above 25 MB → artifact omitted, `meta.json.capture.frozen_html.error = "size_budget_exceeded"`, audit row `capture.frozen_html_failed`. The tier reached is recorded in `meta.json.capture.frozen_html.tier`.
- **Pragmatic-fidelity placeholders** — shadow-DOM hosts and `<iframe>` contents are replaced with visible `[Capsule: not preserved in this view — see page.warc.gz]` markers so a reviewer can see the fidelity boundary instead of finding a silently empty widget. Counts surfaced as `meta.json.capture.frozen_html.{shadow_root_omitted_count, stripped_iframe_count}`.
- **Schema v11** — additive only. [`app/schemas/meta.schema.json`](app/schemas/meta.schema.json) bumps the `schema_version` enum to include `11` and adds the `capture.frozen_html` block (`generated`, `version`, `tier`, `byte_count`, the five counters, `error`). [`postprocess.finalize`](app/postprocess.py) emits `schema_version: 11` going forward. v2–v10 records on disk continue to validate.
- **Audit actions** — `capture.frozen_html_generated` (with tier + counters + sha256) on success; `capture.frozen_html_failed` (with error class + tier + byte_count) on failure. No new audit-row substring overlap with the §8 cookie guard.
- **PDF report row** — new row in the capture-report section, three render branches (generated → `"Generated · {tier} · N inlined, M external"`; error → localised explanation; legacy pre-v11 → dash). New i18n keys in all four bundles under `pdf.report.field.capture.frozen_html.*`.
- **Zero net-new pip dependencies** — pure Playwright + stdlib. The Dockerfile is unchanged.
- **Browsertrix removal still tracked separately** in §16 (the v0.6 in-session WARC writer's robustness across the corpus is the gate for that change, independent of frozen.html).
